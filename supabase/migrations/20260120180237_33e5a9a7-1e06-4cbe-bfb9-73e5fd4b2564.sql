-- Actualizar can_view_requisicion para que autorizadores vean trámites que autorizaron en cualquier etapa
...
BEGIN
  ...
  -- 4. Requisitions with budget rejection where they are the authorizer
  IF _role = 'autorizador' THEN
    RETURN _requisicion.autorizador_id = _user_id 
  ...
  -- Presupuestos can see items at pedido_colocado stage, their own, or ones they rejected
  IF _role = 'presupuestos' THEN
    RETURN _requisicion.estado = 'pedido_colocado' 
      OR _requisicion.solicitado_por = _user_id
      OR _requisicion.rechazado_por_presupuestos_id = _user_id
      OR _requisicion.pedido_autorizado_por = _user_id;
  END IF;
  
  -- Tesoreria can only see items at pedido_autorizado stage or their own
  IF _role = 'tesoreria' THEN
    RETURN _requisicion.estado = 'pedido_autorizado' 
      OR _requisicion.solicitado_por = _user_id
