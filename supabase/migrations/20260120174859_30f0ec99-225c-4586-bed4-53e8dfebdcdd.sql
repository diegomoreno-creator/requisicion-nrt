-- Actualizar la función can_view_requisicion para que autorizadores vean trámites con rechazo de presupuestos
...
BEGIN
  ...
  -- Autorizador can see assigned or created requisitions, or those with budget rejection
  IF _role = 'autorizador' THEN
    RETURN _requisicion.autorizador_id = _user_id 
      OR _requisicion.solicitado_por = _user_id
      OR (_requisicion.autorizador_id = _user_id AND _requisicion.justificacion_rechazo_presupuestos IS NOT NULL);
  END IF;
  ...
  -- Presupuestos can see items at pedido_colocado stage or their own
