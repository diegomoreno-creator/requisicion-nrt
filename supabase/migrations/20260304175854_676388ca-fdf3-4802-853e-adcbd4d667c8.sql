...
-- Update requisiciones UPDATE WITH CHECK policy to allow reviewer to set estado = 'rechazado'
...
-- Same fix for reposiciones: allow reviewer to set estado = 'rechazado'
...
WITH CHECK (
  ...
  OR (has_role(auth.uid(), 'revision'::app_role) AND (revisor_id = auth.uid()) AND (estado = ANY (ARRAY['pendiente_revision'::text, 'pendiente'::text, 'rechazado'::text])))
