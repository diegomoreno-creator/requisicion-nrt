-- Update RLS policy to allow autorizador to update requisitions with presupuestos rejection
-- The autorizador needs to be able to:
-- 1. Update from pedido_colocado to en_licitacion (accept presupuestos rejection)
-- 2. Update from pedido_colocado to pendiente (reject to solicitador)
...
WITH CHECK (
  ...
  -- Autorizador transitions (including new transitions for presupuestos rejection handling)
  OR ((get_user_role(auth.uid()) = 'autorizador'::app_role) AND (autorizador_id = auth.uid()) AND (estado = ANY (ARRAY['aprobado'::requisition_status, 'rechazado'::requisition_status, 'pendiente'::requisition_status, 'en_licitacion'::requisition_status])))
